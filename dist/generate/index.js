...
const tinyglobby_1 = __importDefault(require("tinyglobby"));
const file_1 = require("./file");
const featuresLoader_1 = require("../gherkin/featuresLoader");
...
class TestFilesGenerator {
    constructor(config) {
        this.config = config;
        this.featuresLoader = new featuresLoader_1.FeaturesLoader();
        this.files = [];
        this.logger = new logger_1.TestFilesGeneratorLogger({ verbose: config.verbose });
        if (config.tags)
    ...
    async loadFeatures() {
        ...
        const { files, finalPatterns } = await (0, featuresLoader_1.resolveFeatureFiles)(configDir, features);
    ...
    async loadSteps() {
        ...
        this.logger.logLoadedSteps(files, stepRegistry_1.stepDefinitions);
    ...
    async saveFiles() {
        this.files.forEach((file) => (0, utils_1.saveFileSync)(file.outputPath, file.content));
        this.logger.logGeneratedTestFiles(this.files);
    }
    async clearOutputDir() {
        const pattern = `${tinyglobby_1.default.convertPathToPattern(this.config.outputDir)}/**/*.spec.js`;
        const testFiles = await tinyglobby_1.default.glob(pattern, { expandDirectories: false });
